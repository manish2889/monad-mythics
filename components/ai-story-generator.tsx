      ...
      for (const scene of scenes) {
        try {
          ...
          if (result.success && result.data) {
            ...
            console.error(`Failed to generate image for scene ${scene.id}:`, result.error);
  ...
  const handleMintNFT = async () => {
    ...
    try {
      ...
      const nftMetadata = {
        ...
        attributes: [
          { trait_type: 'Genre', value: selectedGenres.join(', ') || 'Fantasy' },
          { trait_type: 'Format', value: storyFormat },
          { trait_type: 'Setting', value: setting || 'Unknown' },
          { trait_type: 'Has Images', value: generatedImages.length > 0 ? 'Yes' : 'No' },
          { trait_type: 'Image Count', value: generatedImages.length.toString() },
      ...
      toast({
        ...
        description: generatedImages.length > 0 
        ...
        <CardContent>
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid w-full grid-cols-3 bg-gradient-to-r from-orange-500/10 to-red-500/10">
              <TabsTrigger value="input" className="data-[state=active]:bg-orange-500 data-[state=active]:text-white">
                ✨ Craft Tale
              </TabsTrigger>
              <TabsTrigger value="preview" className="data-[state=active]:bg-red-500 data-[state=active]:text-white">
                👁️ Vision
              </TabsTrigger>
              <TabsTrigger value="mint" className="data-[state=active]:bg-pink-500 data-[state=active]:text-white">
                🔮 Immortalize
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="input" className="space-y-6">
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    placeholder="What deeper truths will your tale reveal? Honor, sacrifice, destiny..."
                    value={themes}
                    onChange={(e) => setThemes(e.target.value)}
                    className="bg-slate-900/80 border-indigo-500/30 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-400/20 text-white placeholder:text-slate-400 transition-all duration-300 backdrop-blur-sm"
                  />
                </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    {storyFormats.map((format) => (
                      ...
                      >
                        ...
                        <span className={`text-xs ${storyFormat === format.id ? 'text-white/80' : 'text-muted-foreground'}`}>
                          {format.description}
                        </span>
                      </Button>
                    ))}
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="mt-4 flex items-center space-x-2">
                      <div className={`w-3 h-3 rounded-full transition-all duration-300 ${
                        includeImages ? 'bg-emerald-400 shadow-lg shadow-emerald-400/50' : 'bg-slate-600'
                      }`}></div>
                      <span className="text-xs text-slate-400">
                        {includeImages ? 'Visual mode active' : 'Text-only mode'}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <TabsContent value="preview" className="space-y-4">
              {generatedContent ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {generatedImages.map((image) => (
                          <Card key={image.id} className="overflow-hidden bg-slate-900/60 border-slate-700/50 hover:border-emerald-500/30 transition-all duration-300 group">
                            ...
                            <CardContent className="p-4 bg-slate-800/40">
                              <h4 className="font-semibold text-sm text-emerald-400 mb-1">{image.chapter}</h4>
                              <p className="text-xs text-slate-300 mb-2">{image.description}</p>
                ...
                <div className="text-center py-16 bg-slate-900/40 rounded-xl border-2 border-dashed border-slate-600/40">
                  ...
                  <h3 className="text-xl font-semibold text-slate-300 mb-2">No Tale Forged Yet</h3>
                  <p className="text-slate-400 max-w-md mx-auto leading-relaxed">
                    Return to the ✨ Craft Tale tab to begin weaving your legendary narrative with mystical AI forces.
                  </p>
                </div>
              )}
            </TabsContent>
                  ...
                  <div className="bg-muted/50 p-6 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div className="space-y-2">
                        ...
                        <p>
                          <strong>Story Length:</strong> ~{generatedContent.length}{' '}
                          characters
                        </p>
                      </div>
                      <div className="space-y-2">
                        <p>
                          <strong>Images:</strong> {generatedImages.length} generated
                        </p>
                        <p>
                        ...
                        <p>
                          <strong>Content Type:</strong> {generatedImages.length > 0 ? 'Story + Images' : 'Text Only'}
                        </p>
                      </div>
                    </div>
